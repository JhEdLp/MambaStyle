...
class LatentEditor:
    def __init__(self, domain="human_faces"):
        ...
        if self.domain == "human_faces":
            ...
            self.fs_tensors = {
                name: torch.tensor(np.load(path)).cuda()
                for name, path in self.fs_directions.items()
            }

            #  for style_clip mapper
            self.setup_style_clip()
    ...
    def get_styleclip_mapper_edits(self, start_w, factors, direction):
        latents_to_display = []
        style_clip_net = self.style_clip_nets[direction]
    ...
    def get_fs_edits(self, w, factors, direction):
        boundary = self.fs_tensors[direction]
        bs = w.size(0)
        w_0 = w.reshape(bs, -1)
        boundary = boundary.reshape(1, -1).repeat(bs, 1)
        edits = [(w_0 + factor * boundary).view(bs, -1, 512) for factor in factors]

        return edits

    def setup_style_clip(self):
        self.style_clip_nets = {}

        for direction in ["afro", "angry", "purple_hair", "bobcut", "bowlcut", "mohawk"]:
            mapper_checkpoint_path = os.path.join(
                "pretrained_models_SF/styleclip_mappers",
                f"{direction}.pt",
            )
            ...
            self.style_clip_nets[direction] = style_clip_net
