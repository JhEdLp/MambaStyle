...
class Coach:
    def __init__(self, opts):
        ...
        try:
            ...
            wandb.init(name='Model_Final_1', dir='./', project='GAN_Inv_Cars', resume=False, config=args_wandb,
                       notes='Train all model using a vision mamba model. Using all directions. The training strategy is in a forward pass make inversion an ditability.'
            ...
            artifact.add_file('./training/coach_cars.py')
            # Log the artifact
        ...
        self.scheduler = torch.optim.lr_scheduler.StepLR(self.optimizer, step_size=100000, gamma=self.opts.gamma_scheduler)
        ...
        self.resizer_1 = train_utils.ImageResizer(size=(512, 512), mode='bilinear', antialias=True) 
    ...
    def train_jhon(self):
        ...
        while self.global_step < self.opts.max_steps:
            ...
            with torch.no_grad():
                ...
                latent_noise = torch.randn([self.opts.batch_size, 512], device=self.device)
                w = self.net.decoder.style(latent_noise)# Map z to W space using the mapping network
                w = w.unsqueeze(1).repeat(1, 16, 1)  # Expand w to W+ (18 latent vectors for 1024x1024 resolution)
                ...
                xe = xe[:, :, 68:444, :].clamp(-1, 1)
                x = x[:, :, 68:444, :].clamp(-1, 1)

                x_input = self.resizer.interpolate(x)
                xe_input = self.resizer.interpolate(xe)

                x = self.resizer_1.interpolate(x) 
                xe = self.resizer_1.interpolate(xe)
            ...
            we_hat += self.net.latent_avg[None, ...]
