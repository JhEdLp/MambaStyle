...
class Coach:
    def __init__(self, opts):
        ...
        try:
            ...
            wandb.init(name='Model_Step_by_Step_Adv_1', dir='./', project='GAN_Inv_VSS2D', resume=False, config=args_wandb,
                       notes='Training all model using a vision mamba model. Using all directions. The training strategy is in a forward pass make inversion an editability. '
                       'Adding only direction loss to regularize latent space and aplying inversion only every 20 steps.')
            self.wandb_logger = True

            # Create an artifact
            artifact = wandb.Artifact("Backbone", type="code")
    ...
    def Inversion_step(self, direction):
        ...
        for batch_idx, batch in enumerate(self.train_dataloader):
            ...
            # f_norm = torch.norm(F_hat, p='fro') * 1e-3

            # tv_loss_inv = total_variation_loss(x_hat) * 4

            total_loss = loss_inv + l2_multiscale_inv
        ...
        return total_loss, dict_Inv, [x, x_input, x_hat]
    ...
    def Edition_step(self):
        ...
        # tv_loss = total_variation_loss(y_hat) * 4

        total_loss = loss_edit + l2_multiscale + direction_loss 

        loss_dict['direction_loss'] = direction_loss.item()
        # loss_dict['tv_loss'] = tv_loss.item()
        loss_dict['l2_ms'] = l2_multiscale.item()

        return total_loss, loss_dict, [x_input, xe_input, y_hat_output, y_hat]
    ...
    """ def train_jhon(self):
        ...
        while self.global_step < self.opts.max_steps:
            ...
            # Inversion Step
            loss_inv, inv_dic, attr_real = self.Inversion_step(direction=inv_dir)
            real_img, real_img_1, x_hat = attr_real[0], attr_real[1], attr_real[2]

            # Real and Fake Predictions for Discriminator
            real_img.requires_grad_()  # Enable gradient computation for R1 regularization
            real_img_1.requires_grad_()  # Enable gradient computation for R1 regularization
            real_preds = self.net.discriminator(real_img)
            fake_preds = self.net.discriminator(x_hat)

            # Adversarial loss
            adv_loss_inv = F.softplus(-real_preds).mean() + F.softplus(fake_preds).mean()

            # R1 Regularization
            r1_lambda = 10  # Adjust weight for R1 regularization if needed
            r1_loss = r1_regularization(real_preds, real_img) * r1_lambda

            # Total Inversion Loss
            total_loss_inv = loss_inv + adv_loss_inv + r1_loss

            # Backward Pass for Inversion
            total_loss_inv.backward()

            # Edition Step
            loss_edi, loss_dict, attr_edi = self.Edition_step()
            y_hat_output, y_hat = attr_edi[2], attr_edi[3]

            # Real and Fake Predictions for Discriminator
            real_preds = self.net.discriminator(real_img)
            fake_preds = self.net.discriminator(y_hat)

            # Adversarial Loss
            adv_loss_edi = F.softplus(-real_preds).mean() + F.softplus(fake_preds).mean()

            # Total Edition Loss
            total_loss_edi = loss_edi + adv_loss_edi

            # Backward Pass for Edition
            total_loss_edi.backward()

            # Optimizer and Scheduler Update
            self.optimizer.step()
            self.scheduler.step()

            loss_dict.update(inv_dic)"""
    ...
    def train_jhon(self):
        ...
        while self.global_step < self.opts.max_steps:
            # ----------------------------
            # Inversion Step (First Gradient Accumulation)
            # ----------------------------
            self.optimizer.zero_grad()  # Clear gradients for inversion step

            # Perform Inversion Step with real images from FFHQ
            loss_inv, inv_dic, attr_real = self.Inversion_step(direction=inv_dir)
            real_img, real_img_1, x_hat = attr_real[0], attr_real[1], attr_real[2]

            #R1 regularization
            real_img.requires_grad_()  # Enable gradient computation for R1 regularization
            real_img_1.requires_grad_()
            ...
            # R1 Regularization for Inversion with real images
            r1_lambda = 10  # Weight for R1 regularization
            r1_loss = r1_reg(real_preds, real_img) * r1_lambda

            # Total Inversion Loss
            total_loss_inv = loss_inv + adv_loss_inv + r1_loss

            # Backward pass for inversion step
            total_loss_inv.backward()

            # Optimizer and scheduler step for inversion
            self.optimizer.step()

            # ----------------------------
            # Edition Step (Second Gradient Accumulation)
            # ----------------------------
            self.optimizer.zero_grad()  # Clear gradients for editing step

            # Perform Edition Step with generated images
            loss_edi, loss_dict, attr_edi = self.Edition_step()
            y_hat_output, y_hat = attr_edi[2], attr_edi[3]

            # Real and Fake Predictions for Discriminator using generated images
            real_preds = self.net.discriminator(real_img)
            fake_preds = self.net.discriminator(y_hat)
            adv_loss_edi = F.softplus(-real_preds).mean() + F.softplus(fake_preds).mean()

            # Total Edition Loss (without R1 regularization)
            total_loss_edi = loss_edi + adv_loss_edi

            # Backward pass for edition step
            total_loss_edi.backward()

            # Optimizer and scheduler step for editing
            self.optimizer.step()
            self.scheduler.step()

            # Update the logging dictionary with both inversion and edition losses
            loss_dict.update(inv_dic)

            # Logging related
            if self.global_step % self.opts.image_interval == 0 or (self.global_step < 1000 and self.global_step % 25 == 0):
                if self.wandb_logger:
                    self.parse_and_log_images(None, attr_edi[0], attr_edi[1], attr_edi[2], title='images/train/faces')
    ...
    def configure_optimizers(self):
        ...
        if hasattr(self.net, 'fuser'):
            ...
            params = [{'params': self.net.fuser.parameters(), 'lr': self.opts.lr_fuser}, {'params': self.net.encoder.parameters()}, {'params': self.net.discriminator.parameters()}]
        else:
